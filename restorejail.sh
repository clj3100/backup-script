...
function restoreaction {
	...
	if [[ $restorechoice -eq 1 ]]
		...
		iocage create -r $(freebsd-version | cut -d- -f1,2) -n $newjailname ip4_addr=$newjailip boot=on vnet=$vnet defaultrouter=$defroute
...
datechoice=$(dialog --clear --backtitle "$BACKTITLE" --title "Enter date to restore" --inputbox "ex: 20190215 or last or list" $HEIGHT $WIDTH 2>&1 >/dev/tty)
if [ "$datechoice" == "last" ]
	then
